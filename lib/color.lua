...
-- or `love.graphics.setColor(col:unpack())`
...
function col:hue(h)
  ...
  return col.fromHSV(h % 1, s, v, self.a)
end

---@return color
function col:huesmooth(h)
  local _, s, v = self:hsv()
  return col.fromSHSV(h % 1, s, v, self.a)
end

---@return color
function col:alpha(a)
  return col.fromRGB(self.r, self.g, self.b, a)
end

--- multiplies current alpha by provided value
---@return color
function col:malpha(a)
  return col.fromRGB(self.r, self.g, self.b, self.a * a)
end
...
---@return color
function col:invert()
  return col.fromRGB(1 - self.r, 1 - self.g, 1 - self.b, self.a)
end

---@return color
function col:grayscale()
  local a = self.r * 0.299 + self.g * 0.587 + self.b * 0.114
  return col.fromRGB(a, a, a, self.a)
end

---@return color
function col:hueshift(a)
  local h, s, v = self:hsv()
  return col.fromHSV((h + a) % 1, s, v, self.a)
end
...
function col:temperature(a)
  ...
  local colHue, s, v = self:hsv()
  -- normalize such that 0 = same hue
  local normalizedHue = colHue - targetHue
  -- constrain and wrap to [-0.5 - 0.5]
  local wrapHue = (normalizedHue + 0.5) % 1 - 0.5
  local mixedHue = mix(wrapHue, 0, a)
  local resHue = mixedHue + targetHue

  return col.fromHSV(resHue, s, v, self.a)
end
...
local function genericop(a, b, f, name)
  ...
  if typea == 'number' then
    return col.fromRGB(f(b.r, a), f(b.g, a), f(b.b, a), b.a)
  elseif typeb == 'number' then
    return col.fromRGB(f(a.r, b), f(a.g, b), f(a.b, b), a.a)
  elseif typea == 'color' and typeb == 'color' then
    return col.fromRGB(f(a.r, b.r), f(a.g, b.g), f(a.b, b.b), f(a.a, b.a))
  end
...
---@param a number?
---@return color
function col.fromRGB(r, g, b, a)
  a = a or 1
  return setmetatable({r = r, g = g, b = b, a = a or 1}, colmeta)
end

---@param h number @ hue in range of [0 - 1]; wraps over if necessary
---@param s number
---@param l number
---@param a number?
---@return color
function col.fromHSL(h, s, l, a)
  a = a or 1
...
---@param a number?
---@return color
function col.fromSHSV(h, s, v, a)
  h = h % 1
  return col.fromHSV(h * h * (3 - 2 * h), s, v, a)
end
...
---@param hex string
---@return color
function col.fromHex(hex)
  return col.fromRGB(hexToRGB(hex))
end
